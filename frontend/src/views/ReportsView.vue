...
async function restore() { try { const path = window.desktop?.chooseRestorePath(); if (!path) return; const result = await gateway.restore(path); message.value = result.restarting ? 'Gjenoppretter data og starter programmet på nytt …' : result.message } catch (cause) { message.value = cause instanceof Error ? cause.message : 'Kunne ikke gjenopprette sikkerhetskopien.' } }
onMounted(generate)
</script>

...
export const gateway: TicketGateway = {
  ...
  customerHistory: (phone, page = 0, size = 10, excludeTicketId) => request(`/customers/history?${params({ phone, page, size, excludeTicketId })}`),
  ...
  setStatus: (id, status: TicketStatus, actorId, version, resolutionNote) => request(`/tickets/${id}/status`, { method: 'POST', body: JSON.stringify({ status, actorId, version, resolutionNote }) }),
